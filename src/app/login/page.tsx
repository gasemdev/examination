    ...
    return (
      <main className="app-shell flex h-dvh items-center justify-center overflow-hidden px-4 py-3 sm:px-5 sm:py-5">
        <section className="app-panel w-full max-w-md p-5 md:p-6">
          <div className="flex justify-end gap-2"><LanguageSwitcher /><ThemeToggle /></div><BrandMark href={`/${locale}/login`} compact />
  ...
  return (
    <main className="app-shell min-h-screen overflow-hidden">
      <header className="app-header">
        <div className="mx-auto flex max-w-[790px] items-center justify-between px-5 py-3.5">
          <Link href={`/${locale}/login`} className="flex items-center gap-2.5 text-sm font-bold tracking-tight text-white">
            <span className="grid size-8 place-items-center rounded-full border border-[#7e72ff] text-sm text-[#bdb8ff]"><StarFilled /></span>
            {landing("brand")}
          </Link>
      ...
      <section className="relative mx-auto flex max-w-[680px] flex-col items-center px-5 pb-8 pt-10 text-center md:pt-11">
        <div className="absolute left-1/2 top-0 -z-0 h-64 w-[min(70vw,38rem)] -translate-x-1/2 rounded-full bg-[rgba(92,76,221,0.08)] blur-3xl" />
        ...
        <h1 className="relative mt-5 max-w-[600px] text-3xl font-extrabold leading-[1.18] tracking-tight text-white md:text-[38px]">{landing("headlineBefore")} <span className="text-[#6557eb]">{landing("headlineAccent")}</span><br />{landing("headlineAfter")}</h1>
        <p className="relative mt-4 max-w-[520px] whitespace-pre-line text-[11px] leading-5 text-[var(--muted)] md:text-xs">{landing("description")}</p>
        <p className="relative mt-3 text-[11px] font-semibold text-[var(--warning)]">{landing("savedNotice")}</p>

        <div className="relative mt-4 flex flex-wrap justify-center gap-2 text-[10px] font-semibold">
        ...
        <div className="relative mt-3 flex w-full max-w-[546px] flex-wrap justify-center gap-2 text-[10px] font-bold">
          <span className="rounded-lg bg-[#3c9632] px-4 py-2.5 text-white">👥 13,804&nbsp; {landing("usersStat")}</span>
          <span className="rounded-lg bg-[#367fc7] px-4 py-2.5 text-white">📝 14,686&nbsp; {landing("setsStat")}</span>
          <span className="rounded-lg bg-[#8047c9] px-4 py-2.5 text-white">📗 239&nbsp; {landing("subjectsStat")}</span>
          <span className="rounded-lg bg-[#4039ae] px-4 py-2.5 text-white">🎯 52,196&nbsp; {landing("practiceStat")}</span>
          <span className="rounded-lg bg-[#168a59] px-4 py-2.5 text-white">✓ 1,977,959&nbsp; {landing("answersStat")}</span>
        </div>

        <div className="relative mt-6 grid w-full max-w-[546px] gap-2.5 sm:grid-cols-2">
          <div className="rounded-xl border border-[var(--border)] bg-[rgba(29,30,38,0.82)] px-4 py-3.5"><p className="text-[9px] text-[var(--muted)]">{landing("administration")}</p><p className="mt-1 text-[9px] text-[var(--muted)]">{landing("administrationTrack")}</p><strong className="mt-2 block text-2xl text-[#8e82ff]">85 <small className="text-[10px] text-[var(--muted)]">{landing("days")}</small></strong><div className="mt-3 grid grid-cols-3 gap-2"><span className="rounded-md bg-[#0d0e12] py-2 text-xs font-bold text-white">14<small className="block text-[9px] font-normal text-[var(--muted)]">{landing("hours")}</small></span><span className="rounded-md bg-[#0d0e12] py-2 text-xs font-bold text-white">22<small className="block text-[9px] font-normal text-[var(--muted)]">{landing("minutes")}</small></span><span className="rounded-md bg-[#0d0e12] py-2 text-xs font-bold text-white">19<small className="block text-[9px] font-normal text-[var(--muted)]">{landing("seconds")}</small></span></div></div>
          <div className="rounded-xl border border-[var(--border)] bg-[rgba(29,30,38,0.82)] px-4 py-3.5"><p className="text-[9px] text-[var(--muted)]">{landing("suppression")}</p><p className="mt-1 text-[9px] text-[var(--muted)]">{landing("suppressionTrack")}</p><strong className="mt-2 block text-2xl text-[#8e82ff]">134 <small className="text-[10px] text-[var(--muted)]">{landing("days")}</small></strong><div className="mt-3 grid grid-cols-3 gap-2"><span className="rounded-md bg-[#0d0e12] py-2 text-xs font-bold text-white">14<small className="block text-[9px] font-normal text-[var(--muted)]">{landing("hours")}</small></span><span className="rounded-md bg-[#0d0e12] py-2 text-xs font-bold text-white">22<small className="block text-[9px] font-normal text-[var(--muted)]">{landing("minutes")}</small></span><span className="rounded-md bg-[#0d0e12] py-2 text-xs font-bold text-white">19<small className="block text-[9px] font-normal text-[var(--muted)]">{landing("seconds")}</small></span></div></div>
        </div>
